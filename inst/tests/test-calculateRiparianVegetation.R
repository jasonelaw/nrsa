...
test_that("Data formatted correctly", {
  ...
  ans <- nrsa:::formatRiparianVegetation(x$uid, x$transect, x$transdir, x$parameter, x$result)
...
test_that("calculateVegetationPresence works correctly", local({
  
  expect_that(nrsa:::calculateVegPresence(NA, NA, NA, NA, NA, NA), 
              is_equivalent_to(matrix(NA, 1, 7)))
  expect_that(nrsa:::calculateVegPresence(T, T, T, T, T, T), 
              is_equivalent_to(matrix(T, 1, 7)))
  expect_that(nrsa:::calculateVegPresence(F, F, F, F, F, F), 
              is_equivalent_to(matrix(F, 1, 7)))
  expect_that(nrsa:::calculateVegPresence(F, F, F, F, F, F), 
              is_equivalent_to(matrix(F, 1, 7)))
  expect_that(nrsa:::calculateVegPresence(T, F, T, F, T, F), 
              is_equivalent_to(matrix(T, 1, 7)))
  expect_that(nrsa:::calculateVegPresence(F, T, F, T, F, T), 
...
test_that("calculateVegetationTypes works correctly", {
  ...
  expect_that(nrsa:::calculateVegetationTypes(c('C', 'D'), c('E', 'M'), c(1,1))$result,
