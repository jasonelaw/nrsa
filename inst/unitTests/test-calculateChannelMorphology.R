...
test <- dcast(melt(testData, measure.var = 'RESULT'), UID + TRANSECT ~ PARAMETER)
calculateChannelChar(test[,1], test[,3], test[,4], test[,5])
...
mets <- rbind(tr, cm, ww, trr, td)
check <- arrange(merge(expected, mets, by = c('uid', 'metric'), all = T), metric, uid)
check$diff <- round((check$result.x - check$result.y) / check$result.x, 3)
